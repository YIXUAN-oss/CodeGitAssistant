"use strict";
var CLASS_EXTERNAL_URL = 'externalUrl';
var CLASS_INTERNAL_URL = 'internalUrl';
/**
...
var TextFormatter = /** @class */ (function () {
     ...
     */
    function TextFormatter(commits, repoIssueLinkingConfig, config) {
        ...
        var issueLinkingConfig = repoIssueLinkingConfig !== null
    ...
    TextFormatter.prototype.format = function (input) {
        if (this.config.multiline) {
            var html = [], lines = input.split('\n'), i = void 0, j = void 0, match = void 0;
     ...
     * @returns The formatted HTML.
     */
    TextFormatter.prototype.formatLine = function (input) {
        var tree = {
            type: 7 /* TF.NodeType.Root */,
            start: -1,
            end: input.length,
            contains: []
        };
        ...
        if (this.config.markdown) {
            ...
            while (match = TextFormatter.BACKTICK_REGEXP.exec(input)) {
                var backtick = { index: match.index + match[1].length, run: match[2] }, i = void 0;
                ...
                for (i = backTickStack.length - 1; i >= 0; i--) {
                    if (backTickStack[i].run === backtick.run) {
                        var value = input.substring(backTickStack[i].index + backtick.run.length, backtick.index);
                        if (value.startsWith(' ') && value.endsWith(' ') && /[^ ]/.test(value)) {
                            value = value.substring(1, value.length - 1);
                        }
                        TextFormatter.insertIntoTree(tree, {
                            type: 1 /* TF.NodeType.Code */,
        ...
        if (this.config.urls) {
            ...
            while (match = TextFormatter.URL_REGEXP.exec(input)) {
                var url = match[0];
                var suffix = url.substring(url.length - 1);
        ...
        if (this.config.commits) {
            ...
            var _loop_1 = function () {
                var hash = match[0].toLowerCase();
                var commit = this_1.commits.find(function (commit) { return commit.hash.toLowerCase().startsWith(hash); });
                if (commit) {
                    TextFormatter.insertIntoTreeIfNoOverlap(tree, {
                        type: 2 /* TF.NodeType.CommitHash */,
                        commit: commit.hash,
                        start: match.index,
        ...
        if (this.config.markdown) {
            // Emphasis
            var emphasisTokens = [], emphasisRuns = [];
            var runLength = void 0, whitespaceBefore = void 0, whitespaceAfter = void 0, punctuationBefore = void 0, punctuationAfter = void 0, isLeft = void 0, isRight = void 0, isOpen = void 0, isClosed = void 0;
            TextFormatter.EMPHASIS_REGEXP.lastIndex = 0;
            while (match = TextFormatter.EMPHASIS_REGEXP.exec(input)) {
                var prev = 0, cur = 1, next = 2, index = match.index;
                var seq = [match[1]];
                seq.push.apply(seq, match[2].split(''));
                seq.push(match[3]);
                if (seq[0].startsWith('\\')) {
                    if (seq[0].length % 2 === 1) {
                        index += seq[0].length;
            ...
            var emphasisStack = [];
            var stackMatch = void 0;
            for (var i = 0; i < emphasisTokens.length; i++) {
                var delimiter = emphasisTokens[i];
                var run_1 = emphasisRuns[delimiter.run];
                if (run_1.close && (stackMatch = TextFormatter.findOpenEmphasis(delimiter, run_1, emphasisRuns, emphasisStack)) > -1) {
                    TextFormatter.insertIntoTree(tree, {
                        type: emphasisRuns[emphasisStack[stackMatch].run].type === "*" /* TF.EmphasisDelimiterType.Asterisk */ ? 0 /* TF.NodeType.Asterisk */ : 8 /* TF.NodeType.Underscore */,
        ...
        var rec = function (node) {
            ...
            switch (node.type) {
                case 0 /* TF.NodeType.Asterisk */:
                case 8 /* TF.NodeType.Underscore */:
                    ...
                    html.push('</em>');
                    break;
                case 3 /* TF.NodeType.DoubleAsterisk */:
                case 4 /* TF.NodeType.DoubleUnderscore */:
                    ...
                    html.push('</strong>');
                    break;
                case 6 /* TF.NodeType.Plain */:
                    html.push(escapeHtml(node.value));
                    break;
                case 1 /* TF.NodeType.Code */:
                    html.push('<code>', escapeHtml(node.value), '</code>');
                    break;
                case 2 /* TF.NodeType.CommitHash */:
     ...
     */
    TextFormatter.registerCustomEmojiMappings = function (mappings) {
        var validShortcodeRegExp = /^:[A-Za-z0-9-_]+:$/;
     ...
     */
    TextFormatter.findOpenEmphasis = function (delimiter, run, runs, stack) {
     ...
     */
    TextFormatter.combineNestedEmphasis = function (tree) {
        tree.contains.forEach(TextFormatter.combineNestedEmphasis);
        if (tree.contains.length === 1 && tree.type === tree.contains[0].type && (tree.type === 0 /* TF.NodeType.Asterisk */ || tree.type === 8 /* TF.NodeType.Underscore */) && tree.start + 1 === tree.contains[0].start && tree.contains[0].end === tree.end - 1) {
            tree.type = tree.type === 0 /* TF.NodeType.Asterisk */
                ? 3 /* TF.NodeType.DoubleAsterisk */
                : 4 /* TF.NodeType.DoubleUnderscore */;
     ...
     */
    TextFormatter.insertIntoTree = function (tree, node) {
        var firstChildIndexOfNode = -1, lastChildIndexOfNode = -1, curNode;
        for (var i = 0; i < tree.contains.length; i++) {
     ...
     * @param node The node to insert.
     */
    TextFormatter.insertIntoTreeIfNoOverlap = function (tree, node) {
        var curNode, insertAtIndex = tree.contains.length;
        for (var i = 0; i < tree.contains.length; i++) {
            curNode = tree.contains[i];
     ...
     */
    TextFormatter.isInTree = function (tree, start, end) {
        return tree.contains.some(function (node) { return (node.start <= start && start <= node.end) || (node.start <= end && end <= node.end) || (start <= node.start && node.end <= end); });
    };
...
var ISSUE_LINKING_ARGUMENT_REGEXP = /\$([1-9][0-9]*)/g;
/**
...
function generateIssueLinkFromMatch(match, issueLinking) {
    return match.length > 1
        ? issueLinking.url.replace(ISSUE_LINKING_ARGUMENT_REGEXP, function (placeholder, index) {
