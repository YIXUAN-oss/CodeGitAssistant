...
class Branch {
    ...
    static drawPath(svg, path, isCommitted, colour) {
...
class Vertex {
     ...
     * 绘制顶点
     */
    draw(svg, config, expandOffset, commits) {
        if (this.onBranch === null)
            return;
...
export class GitGraphRenderer {
    ...
    updateConfig(updates) {
        if (updates.grid) {
            this.config.grid = Object.assign(Object.assign({}, this.config.grid), updates.grid);
        }
        if (updates.colours) {
     ...
     * 加载提交数据并构建图形
     */
    loadCommits(commits, commitHead, _branchGraph) {
     ...
     * 渲染图形到 SVG
     */
    render(svg, expandedCommitIndex = -1) {
        ...
        for (let i = 0; i < this.vertices.length; i++) {
            this.vertices[i].draw(group, this.config, expandedCommitIndex > -1 && i > expandedCommitIndex, this.commits);
        }
...
//# sourceMappingURL=git-graph-renderer.js.map