...
export class BranchTreeComponent {
    ...
    checkOperationStatus() {
        ...
        if (!this.data)
            return;
        const currentBranch = (_a = this.data.branches) === null || _a === void 0 ? void 0 : _a.current;
        const currentLogCount = ((_c = (_b = this.data.log) === null || _b === void 0 ? void 0 : _b.all) === null || _c === void 0 ? void 0 : _c.length) || 0;
        const commandHistory = this.data.commandHistory || [];
        // 检查创建分支状态
        if (this.isCreatingBranch && this.createRequestTimestamp) {
            const matchedEntry = commandHistory.find((item) => item.command === 'git-assistant.createBranch' &&
    ...
    getHtml() {
        ...
        const localBranches = this.data.branches.all.filter((b) => !b.startsWith('remotes/'));
        const remoteBranches = this.data.branches.all.filter((b) => b.startsWith('remotes/'));
        const currentBranch = ((_b = this.data.branches) === null || _b === void 0 ? void 0 : _b.current) || null;
        return `
    ...
    getStatusMessage() {
        ...
        if (this.isSwitchingBranch)
            return `正在切换到分支 "${this.switchingBranchName}"...`;
        if (this.switchResult === 'success')
            return `已成功切换到分支 "${this.switchingBranchName}"`;
        if (this.switchResult === 'error')
            return '切换分支失败';
        if (this.isMergingBranch)
            return `正在合并分支 "${this.mergingBranchName}"...`;
        if (this.mergeResult === 'success')
            return `已成功合并分支 "${this.mergingBranchName}"`;
        if (this.mergeResult === 'error')
            return '合并分支失败';
        return '';
    }
    getLocalBranchesHtml(branches, currentBranch) {
        return `
    ...
    getRemoteBranchesHtml(branches) {
        return `
            <div class="branch-section">
                <div class="branch-section-header">
