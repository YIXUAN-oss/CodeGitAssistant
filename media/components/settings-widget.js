 ...
 */
import { SVG_ICONS, escapeHtml, alterClass, observeElemScroll, formatCommaSeparatedList, getRepoName, addListenerToClass, ELLIPSIS, CLASS_ACTIVE, CLASS_TRANSITION, CLASS_LOADING, arraysStrictlyEqualIgnoringOrder } from '../utils/vscode-git-utils.js';
import { Dialog, DialogInputType } from './dialog.js';
...
const updateGlobalViewState = (key, value) => {
    if (typeof window.updateGlobalViewState === 'function') {
        window.updateGlobalViewState(key, value);
    }
};
const sendMessage = (msg) => {
    if (typeof window.vscode !== 'undefined' && window.vscode.postMessage) {
        window.vscode.postMessage(msg);
    }
};
const runAction = (action, actionName) => {
    if (typeof window.runAction === 'function') {
        window.runAction(action, actionName);
    }
    else {
        sendMessage(action);
    }
...
function getOnRepoLoadShowCheckedOutBranch(value) {
    return value === true || value === 1 || value === 'enabled';
}
function getOnRepoLoadShowSpecificBranches(value) {
    return Array.isArray(value) ? value : [];
}
function getShowStashes(value) {
    return value === true || value === 1 || value === 'enabled';
}
function getShowTags(value) {
    return value === true || value === 1 || value === 'enabled';
}
function getIncludeCommitsMentionedByReflogs(value) {
    return value === true || value === 1 || value === 'enabled';
}
function getOnlyFollowFirstParent(value) {
    return value === true || value === 1 || value === 'enabled';
}
// Types for Git Graph
var BooleanOverride;
(function (BooleanOverride) {
    BooleanOverride[BooleanOverride["Default"] = 0] = "Default";
    BooleanOverride[BooleanOverride["Enabled"] = 1] = "Enabled";
    BooleanOverride[BooleanOverride["Disabled"] = 2] = "Disabled";
})(BooleanOverride || (BooleanOverride = {}));
var PullRequestProvider;
(function (PullRequestProvider) {
    PullRequestProvider[PullRequestProvider["Bitbucket"] = 0] = "Bitbucket";
    PullRequestProvider[PullRequestProvider["GitHub"] = 1] = "GitHub";
    PullRequestProvider[PullRequestProvider["GitLab"] = 2] = "GitLab";
    PullRequestProvider[PullRequestProvider["Custom"] = 3] = "Custom";
})(PullRequestProvider || (PullRequestProvider = {}));
var GitConfigLocation;
(function (GitConfigLocation) {
    GitConfigLocation[GitConfigLocation["Local"] = 0] = "Local";
    GitConfigLocation[GitConfigLocation["Global"] = 1] = "Global";
})(GitConfigLocation || (GitConfigLocation = {}));
/**
...
export class SettingsWidget {
                ...
                if (pullRequestConfig !== null) {
                    const provider = escapeHtml((pullRequestConfig.provider === PullRequestProvider.Bitbucket
                        ...
                        : pullRequestConfig.provider === PullRequestProvider.Custom
                            ? (((_e = pullRequestConfig.custom) === null || _e === void 0 ? void 0 : _e.name) || 'Custom')
            ...
            if (this.config !== null) {
                document.getElementById('editUserDetails').addEventListener('click', () => {
                    ...
                    getDialog().showForm('Set the user name and email used by Git to record the Author and Committer of commit objects:', [
                        { type: DialogInputType.Text, name: 'User Name', default: (_b = (_a = userName.local) !== null && _a !== void 0 ? _a : userName.global) !== null && _b !== void 0 ? _b : '', placeholder: null },
                        { type: DialogInputType.Text, name: 'User Email', default: (_d = (_c = userEmail.local) !== null && _c !== void 0 ? _c : userEmail.global) !== null && _d !== void 0 ? _d : '', placeholder: null },
    ...
    showIssueLinkingDialog(defaultIssueRegex, defaultIssueUrl, defaultUseGlobally, isEdit) {
        ...
        if (!isEdit && defaultIssueRegex === null && defaultIssueUrl === null) {
            ...
            const commits = this.view.getCommits ? this.view.getCommits() : [];
        ...
        getDialog().showForm(html, [
            ...
            const issueRegex = values[0].trim(), issueUrl = values[1].trim(), useGlobally = values[2];
    ...
    showCreatePullRequestIntegrationDialog1(config) {
        ...
        if (config.provider === PullRequestProvider.Custom) {
            if (!providerOptions.some((provider) => { var _a; return provider.name === ((_a = config.custom) === null || _a === void 0 ? void 0 : _a.name); })) {
        ...
        getDialog().showForm('Configure "Pull Request Creation" Integration (Step&nbsp;1/2)', [
            ...
            if (newProvider > 3)
                newProvider = PullRequestProvider.Custom;
    ...
    showCreatePullRequestIntegrationDialog2(config) {
        ...
        const updateConfigWithFormValues = (values) => {
            ...
            config.destProjectId = config.provider === PullRequestProvider.GitLab ? values[5] : '';
            const destBranch = values[config.provider === PullRequestProvider.GitLab ? 6 : 5];
        ...
        getDialog().showForm('Configure "Pull Request Creation" Integration (Step&nbsp;2/2)', inputs, 'Save Configuration', (values) => {
