...
export class GitCommandReferenceComponent {
    ...
    attachEventListeners() {
        // 搜索
        const searchInput = this.container.querySelector('#command-search');
        if (searchInput) {
    ...
    filterCategories(categories) {
        ...
        return categories.map(category => {
            const filteredCommands = category.commands.filter(cmd => cmd.command.toLowerCase().includes(searchLower) ||
                cmd.description.toLowerCase().includes(searchLower) ||
                (cmd.example && cmd.example.toLowerCase().includes(searchLower)));
            if (filteredCommands.length === 0) {
                return null;
            }
            return Object.assign(Object.assign({}, category), { commands: filteredCommands });
        }).filter((cat) => cat !== null);
    }
    getCategories() {
        return [
...
//# sourceMappingURL=git-command-reference.js.map